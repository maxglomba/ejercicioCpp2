...
public:
	Luchador() {};
	virtual ~Luchador() {
		cout << "Eliminando punteros de Luchador";
		delete ptr_poder;
		delete ptr_patada;
		delete ptr_especial;
	};
...
void menu() {
	...
	if (p1 != 0 && p2 != 0) {
		...
		*/
		...
		if (isKakashi = dynamic_cast<Kakashi*>(jugador1)) {
			isKakashi->~Kakashi();
		}else if (isNaruto = dynamic_cast<Naruto*>(jugador1)) {
			isNaruto->~Naruto();
		}else if (isSasuke = dynamic_cast<Sasuke*>(jugador1)) {
			isSasuke->~Sasuke();
		}
		*/
	}
